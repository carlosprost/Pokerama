...
function cargarPares(){
    ...
    }else if (parCatorce < 2) {
        cargarPar(parCatorce);

    }else if (parQuince < 2) {
        cargarPar(parQuince);

    }else if (parDieciseis < 2) {
        cargarPar(parDieciseis);

    }else if (parDiecisiete < 2) {
        cargarPar(parDiecisiete);

    }else if (parDieciocho < 2) {
        cargarPar(parDieciocho);
...
function revisarPares(clave) {
    return revisarPar(parUno, clave)    || 
           ...
           revisarPar(parTrece, clave)  || 
           revisarPar(parCatorce, clave)   ||
           revisarPar(parQuince, clave)  || 
           revisarPar(parDieciseis, clave)   || 
           revisarPar(parDiecisiete, clave)   || 
           revisarPar(parDieciocho, clave);
}
...
function noSonPares(){
    return parUno.length < 2    || 
           ...
           parCatorce.length < 2   ||
           parQuince.length < 2  || 
           parDieciseis.length < 2   || 
           parDiecisiete.length < 2   || 
           parDieciocho.length < 2; 
}
